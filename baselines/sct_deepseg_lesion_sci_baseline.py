...
def process_image_label_pair(image_path, label_path, output_folder):
    ...
    output_segmentation_path = os.path.join(output_folder, filename)
    ...
    subprocess.run(['sct_deepseg', '-i', image_path, '-task', 'seg_sc_lesion_t2w_sci', '-o', output_segmentation_path], env=env)
