...
def set_array(nii: Nifti1Image, arr: np.ndarray, inplace=False) -> Nifti1Image:
...
def get_max_affine_and_shape(points: np.ndarray, affines, min_spacing=None, dtype: type = float, verbose=False):
    ...
    print("The new origin is as", affine[:3, 3]) if verbose else None
    print("The optimal rotation came from file number ", opt_id) if verbose else None
...
def compute_crop_slice(nii: Nifti1Image, minimum=0, dist=0):
    ...
    ex_slice = tuple([slice(x0, x1 + 1), slice(y0, y1 + 1), slice(z0, z1 + 1)])
    return ex_slice


def dilate_msk(msk_i_data: np.ndarray, mm: int = 5, connectivity: int = 3, inplace=False):
...
def n4_bias_field_correction(
    ...
    import ants  # install antspyx not ants!
...
def buffer_reference(path, bias_field: bool, crop=False):
    ...
    if bias_field:
        reference = n4_bias_field_correction(nib.load(path), crop)
    else:
        reference = get_array(nib.load(path))
...
def main(
    images: list[str] | list[Path] | list[nib.Nifti1Image],
    output: str | None,
    ...
    corners = []
    affines = []
    niis: list[nib.Nifti1Image] = []
    print("### loading ###") if verbose else None
    for f_name in images:
        if isinstance(f_name, str) or isinstance(f_name, Path):
    ...
    for id, item in enumerate(combinations, 1):
        print(f"{id:2}/{len(combinations):2} ramp stitching", end="\r") if verbose else None
        ...
        if overlap.sum() > 0:
            ...
            arr_1[overlap] = distance_transform_edt(1.0 - arr_2_opend)[overlap]
            arr_2[overlap] = distance_transform_edt(1.0 - arr_1_opend)[overlap]
            ...
            if arr_1_full.max() != 1:
                ...
                if kick_out_fully_integrated_images:
                    images.pop(item[1])
            if arr_1_full.max() != 1 or arr_2_full.max() != 1:
                if kick_out_fully_integrated_images:
    ...
    if is_segmentation:
        occupancy_arr = np.round_(occupancy_arr)  # TODO assuming only two intersecting regions
...
if __name__ == "__main__":
    ...
    help = "fits the histogram, for the previous in the file list. "
    parser.add_argument(
        "-hists",
        "--match_histogram",
        default=False,
    ...
    help = "crop with generated n4_bias_field_correction"
    parser.add_argument(
        "-bias_crop",
    ...
    help = "crop black spaces"
    parser.add_argument("-crop", "--crop", default=False, action="store_true", help=help)
    parser.add_argument("-v", "--verbose", default=True, action="store_true")
    help = "intersecting images are bended together by there distance from vowels that are not intersecting. This flag saves the blending as a 4d nii."
    parser.add_argument("-sr", "--store_ramp", default=False, action="store_true", help=help)
    help = "If two images cut in a way, that would leave a thin slice of less than x voxels pixel, it will not be considered for the ramp calculation."
    parser.add_argument("-ramp_e", "--ramp_edge_min_value", type=int, default=5, help=help)
    help = "all values below will be set to min_value. (MRI=0, CT<=-1024)"
    parser.add_argument("-min_value", "--min_value", type=int, default=0, help=help)
